...
class MauzoController extends Controller
{
    ...
    private function storeRegularSale(Request $request, $companyId)
    {
        return DB::transaction(function () use ($request, $companyId) {
            ...
            $mauzo->created_at = $saleDate;
            $mauzo->updated_at = $saleDate;
            
            // Add user tracking (who made the sale)
            if (Auth::guard('web')->check()) {
                $mauzo->user_id = Auth::id();
                $mauzo->mfanyakazi_id = null;
            } elseif (Auth::guard('mfanyakazi')->check()) {
                $mauzo->user_id = null;
                $mauzo->mfanyakazi_id = Auth::guard('mfanyakazi')->id();
            }
    ...
    private function storeLoan(Request $request)
    {
        ...
        return DB::transaction(function () use ($request, $companyId) {
            ...
            // Add activity logging for loan
            ActivityHelper::log('debt_created', "Deni jipya: {$deni->jina_mkopaji} - {$bidhaa->jina} - Tsh " . number_format($deni->jumla, 0), $deni, $deni->jumla);
    ...
    public function storeKikapuLoan(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                // Add activity logging for each loan item
                ActivityHelper::log('debt_created', "Deni jipya: {$request->jina_mkopaji} - {$bidhaa->jina} - Tsh " . number_format($jumla, 0), $debt, $jumla);
            }
